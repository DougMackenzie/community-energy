...
const HomePage = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Scenario explainers for community leaders */}
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-2">
          Understanding the Four Scenarios
        </h2>
        <p className="text-gray-600 mb-6">
          When evaluating a data center proposal, it's important to understand what operating conditions
          are being proposed. Here's what each scenario means:
        </p>

        <div className="space-y-4">
          {/* Baseline */}
          <div className="p-4 rounded-lg border-l-4 bg-gray-50" style={{ borderLeftColor: SCENARIOS.baseline.color }}>
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-semibold text-gray-900">Baseline: No New Data Center</h3>
                <p className="text-sm text-gray-600 mt-1">
                  What happens to your bill with normal inflation and infrastructure aging, even without
                  any new data center. This is your comparison point.
                </p>
              </div>
              <div className="text-right ml-4">
                <p className="text-lg font-bold text-gray-700">${baselineFinalBill.toFixed(0)}/mo</p>
          ...
          {/* Firm Load */}
          <div className="p-4 rounded-lg border-l-4 bg-red-50" style={{ borderLeftColor: SCENARIOS.unoptimized.color }}>
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-semibold text-gray-900">Firm Load: No Flexibility</h3>
                <p className="text-sm text-gray-600 mt-1">
                  The data center runs 24/7 at full power with no ability to reduce during peak times.
                  This requires the most infrastructure investment and creates the highest costs.
                </p>
                <p className="text-xs text-gray-500 mt-2">
                  <strong>Typical of:</strong> Older data centers, operators unwilling to participate in demand response
                </p>
              </div>
              <div className="text-right ml-4">
                <p className="text-lg font-bold text-red-600">${summary.finalYearBills.unoptimized.toFixed(0)}/mo</p>
                <p className={`text-xs ${firmLoadDiff >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {firmLoadDiff >= 0 ? '+' : ''}{firmLoadDiff.toFixed(2)} vs baseline
                </p>
              </div>
          ...
          {/* Flexible */}
          <div className="p-4 rounded-lg border-l-4 bg-amber-50" style={{ borderLeftColor: SCENARIOS.flexible.color }}>
            <div className="flex items-start justify-between">
              <div>
              ...
              <div className="text-right ml-4">
                <p className="text-lg font-bold text-amber-600">${summary.finalYearBills.flexible.toFixed(0)}/mo</p>
                <p className={`text-xs ${flexLoadDiff >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {flexLoadDiff >= 0 ? '+' : ''}{flexLoadDiff.toFixed(2)} vs baseline
                </p>
              </div>
          ...
          {/* Dispatchable */}
          <div className="p-4 rounded-lg border-l-4 bg-green-50 border-2 border-green-200" style={{ borderLeftColor: SCENARIOS.dispatchable.color }}>
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-semibold text-gray-900">Optimized: Flexibility + Onsite Generation</h3>
                <p className="text-sm text-gray-600 mt-1">
                  In addition to demand response, the data center has backup generators that can provide
                  power to the grid during emergencies. This creates the most benefit for all ratepayers.
                </p>
                <p className="text-xs text-gray-500 mt-2">
                  <strong>Community benefit:</strong> Acts as a grid resource during extreme weather or outages
                </p>
              </div>
              <div className="text-right ml-4">
                <p className="text-lg font-bold text-green-600">${summary.finalYearBills.dispatchable.toFixed(0)}/mo</p>
                <p className={`text-xs ${dispatchableDiff >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {dispatchableDiff >= 0 ? '+' : ''}{dispatchableDiff.toFixed(2)} vs baseline
                </p>
              </div>
